...
def run(agent1_name, agent2_name, map_name, round_num, max_step, random_pos=False):
    ...
    agent1.terminate()
    agent2.terminate()
    return side1_win_times, side2_win_times, draw_times, len(agent1_crash_list), len(agent2_crash_list), len(
...
if __name__ == "__main__":
    ...
    for x in range(len(agent_list)):
        for y in range(len(agent_list)):
            ...
            agent1_win_rounds, agent2_win_rounds, draw_rounds, agent1_crash_rounds, agent2_crash_rounds, agent1_timeout_rounds, agent2_timeout_rounds, agent1_launch_failure_rounds, agent2_launch_failure_rounds = run(
                agent1, agent2, map_name, int(round_num / 2), max_step)
            # record
            ...
            summary_table = pd.DataFrame(
                columns=['agent', 'score', 'win_rate', 'total_rounds', 'win_rounds', 'draw_rounds', 'loss_rounds',
                         'crash_rounds', 'timeout_rounds', 'launch_failure_rounds'])
            for z in range(len(agent_list)):
                if total_rounds_per_agent[z] == 0:
                    win_rate = 0
                else:
                    win_rate = win_rounds_per_agent[z] / total_rounds_per_agent[z]
                score = SCORE_WIN * win_rounds_per_agent[z] + SCORE_DRAW * draw_rounds_per_agent[z] + SCORE_LOSS * (
                            total_rounds_per_agent[z] - win_rounds_per_agent[z] - draw_rounds_per_agent[z])
                summary_table = summary_table.append(pd.DataFrame({'agent': [agent_list[z]],
                                                                   ...
                                                                   'loss_rounds': [
                                                                       total_rounds_per_agent[z] - win_rounds_per_agent[
                                                                           z] - draw_rounds_per_agent[z]],
                                                                   'crash_rounds': [crash_rounds_per_agent[z]],
                                                                   'timeout_rounds': [timeout_rounds_per_agent[z]],
                                                                   'launch_failure_rounds': [
                                                                       launch_failed_rounds_per_agent[z]]}))
